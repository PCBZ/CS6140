...
class CustomRNNCell(nn.Module):
    ...
    def forward(self, input: torch.Tensor, hidden: torch.Tensor) -> torch.Tensor:
        ...
        input_transformed = input @ self.W_ih
        # 2. Compute hidden transformation: hidden @ W_hh + b_hh
        hidden_transformed = hidden @ self.W_hh + self.b_hh
        # 3. Add them together and apply tanh activation
        new_hidden = torch.tanh(input_transformed + hidden_transformed)
...
class LSTMCell(nn.Module):
    ...
    def forward(self, input: torch.Tensor, states: tuple[torch.Tensor, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        forget_gate = torch.sigmoid(input @ self.W_if + hidden @ self.W_hf + self.b_f)
        input_gate = torch.sigmoid(input @ self.W_ii + hidden @ self.W_hi + self.b_i)
        input_node = torch.tanh(input @ self.W_in + hidden @ self.W_hn + self.b_n)
        output_gate = torch.sigmoid(input @ self.W_io + hidden @ self.W_ho + self.b_o)
        # 2. Update cell state: new_cell = forget_gate * old_cell + input_gate * candidate
        new_cell = forget_gate * cell + input_gate * input_node
...
class Encoder(nn.Module):
    def __init__(self, input_size: int, embed_size: int, hidden_size:int, num_layers:int=1, rnn_type:str='lstm'):
        ...
        # TODO: Initialize RNN (CustomRNN) or LSTM (CustomLSTM) based on rnn_type
        # Hint: Use batch_first=True for easier handling
        if rnn_type == 'rnn':
            self.rnn = CustomRNN(input_size=embed_size, hidden_size=hidden_size, num_layers=num_layers, batch_first=True)
        elif rnn_type == 'lstm':
            self.rnn = CustomLSTM(input_size=embed_size, hidden_size=hidden_size, num_layers=num_layers, batch_first=True)
        else:
...
class Decoder(nn.Module):
    ...
    def forward(self, input: torch.Tensor, hidden: torch.Tensor, encoder_outputs: torch.Tensor=None) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        packed_outputs, hidden = self.rnn(embedded, hidden)
        # 3. Apply output projection (need to squeeze the rnn output dim=1)
        output = self.out(packed_outputs.squeeze(1))
...
def create_rnn_model(src_vocab_size: int, tgt_vocab_size: int, embed_size:int=256, 
    ...
    decoder = Decoder(input_size=tgt_vocab_size, embed_size=embed_size, hidden_size=hidden_size, num_layers=num_layers, rnn_type="rnn")
